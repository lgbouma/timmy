...
\begin{abstract}
  ...
  star and validated planet are therefore $35^{+11}_{-5}$ million
  years old.  The planet is warm ($P = 8.3\,{\rm d}$) and slightly
  smaller than Jupiter.  Combining the transit photometry with a prior on the
  stellar parameters derived from the cluster color-magnitude diagram,
  we find that the transits are grazing with $b=0.93\pm0.01$ and
  $R_{\rm p} = 0.79 \pm 0.07 \,R_{\rm Jup}$.  From radial velocity
...
growth over the next 10--100 million years through giant impacts
\citep[{\it
...
small.  We considered this result worth reporting because of the
planet's youth.
...
\begin{figure*}[!t]
    ...
    Starspot-induced variability is the dominant signal; flares
    are shown with gray crosses.  Dashed lines indicate the five
    transits observed by TESS over Sectors 10 and 11.  {\it Middle}: Zoomed windows of
    individual transits.  Gray lines are the best-fit transit model,
    which includes a local quadratic trend in each window.
    {\it Bottom}: Phase-folded TESS transit. Gray points are 2-minute PDCSAP flux
    measurements with the local spot-induced variation removed.  Black
    points are binned to 6-minute intervals. The black error-bar shows
    the median uncertainty for the black points.  The gray line is the
    best-fit model to the combined TESS and ground-based data
...
% 	\begin{center}
% 		\leavevmode
% 		\includegraphics[width=.85\textwidth]{f1.pdf}
% 	\end{center}
% 	\vspace{-0.7cm}
% 	\caption{
...
% 	\begin{center}
% 		\leavevmode
% 		\includegraphics[width=.47\textwidth]{f2a.pdf}
% 	\end{center}
% 	\vspace{-0.7cm}
% 	\caption{
%     {\bf Phase-folded TESS transit.} Gray points are 2-minute PDCSAP flux
%     measurements with the local spot-induced variation removed.  Black
%     points are binned to 6-minute intervals. The black error-bar shows
%     the median uncertainty for the black points.  The gray line is the
%     best-fit model to the combined TESS and ground-based data
...
\begin{figure}[!t]
    ...
    White circles are resolved Gaia sources with $T<16$ (brighter stars
    ...
    ascension are shown.
    Two stars of interest are ``Star A'' and ``Star B'', which were eventually excluded as being possible sources of the transits.
    %{\it Bottom:} Digitized Sky Survey $R$-band
   % image of the same field, with a linear grayscale. 
		\label{fig:scene}
	}
\end{figure}
...
white circles in Figure~\ref{fig:scene}, overlaid on 
the TESS image.  Given its galactic latitude of $b=-6^\circ$, it is
...
\begin{itemize}
  ...
  \item Star A $\equiv$ TIC 847769574 ($T=14.6$), $2.3''$ west. The
    ...
    \tn and is separated from it by $6.6\pm 0.1\,{\rm pc}$.
    It is therefore likely an \cn\ member, but unlikely to be a bound
    binary compnaion.
  %
...
\texttt{2020-07-08}. } value is available.  We suspect that the photometric
...
\begin{figure}[!t]
	\begin{center}
		\leavevmode
		\includegraphics[width=.47\textwidth]{f2b.pdf}
	\end{center}
	\vspace{-0.7cm}
	\caption{
		{\bf Ground-based follow-up photometry.} The data were acquired
		using the 0.36m at El Sauce and the 0.40m ASTEP400 at Dome C. Black points are
		binned to 10-minute intervals.  The gray line is the best-fit
		model to the combined TESS and ground-based data. Models show
...
\edit1{Phil + Karen TODO: please correct as appropriate}
...
observed eclipse depth.  We take the lack of a transit depth variation
along these light curves as evidence that \tn\ is the source of the
transit signal to within $\approx2.0''$.  The lack of variation of
transit depth with aperture location is consistent with \tn\ being the
source of transits, because the uncertainty of the transit depth
measurements ($\approx 15\%$) is much greater than the maximum
dilution from Star A ($\approx 1\%$).
...
\edit1{Amaury TODO: please correct as appropriate}
...
\begin{figure}[!t]
	\begin{center}
		\leavevmode
		\subfloat{
			\includegraphics[width=0.44\textwidth]{f6.pdf}
		}
		
		\vspace{-0.5cm}
		\subfloat{
			\includegraphics[width=0.46\textwidth]{f3.pdf}
		}
	\end{center}
	\vspace{-0.6cm}
	\caption{
    ...
    injection-recovery experiments. Star A ($\Delta
    ...
    separated from \tn\ by $6.6\pm 0.1\,{\rm pc}$.  {\it Middle:}
    Radial velocity (RV) measurements, with best-fit instrument
    offsets and jitter terms included.  The expected scatter from
    starspots based on $v\sin i$ and the photometric modulation
    amplitude is of order 300m$\,$s$^{-1}$.  {\it Bottom:} RV
    measurements phased to the orbital ephemeris of \pn.  The planet
    is not detected.  The black line shows a circular Keplerian orbit
    representing the $3\sigma$ upper mass limit.
		\label{fig:followup}
	}
\end{figure}
...
\edit1{George TODO: please describe the formal secondary line search
in more depth in this paragraph} We searched for secondary lines in
...
wavelength calibrated spectra.  To measure radial velocities, \edit1{
Rafael/Paula TODO: what procedure was used?  What software packages?}.  The
velocities are given in Table~\ref{tab:rvs}, and shown in
Figure~\ref{fig:followup}.
...
600--950$\,$nm at a spectral resolution of $\approx 80{,}000$.
...
Figure~\ref{fig:hr} suggests that the membership census of \cn\
...
roughly $0.7$--$7\,M_\odot$.  The PMS K and M dwarf models are
bluer than observed in the Gaia photometry.  This discrepancy was noted and discussed at
length by \citet{choi_mesa_2016}.  The most plausible explanation
seems to be that strong magnetic fields in low-mass pre-main-sequence
stars inhibit convection and produce a high filling factor of
starspots \citep[{\it
...
CHIRON spectra (Section~\ref{subsec:chiron}).
We then calculated the stellar radius and reddening
following \citet{stassun_accurate_2017}.  We first derived the
bolometric flux by combining available broadband magnitudes from Gaia,
Tycho-2, APASS, 2MASS, and WISE.  We then fitted the SED with the
...
procedure yielded $A_{\rm V} = 0.20\pm0.03$, which agrees with the
average from the \cn\ isochrone fits of \citet{randich_gaiaeso_2018}.
Combining the spectroscopic effective temperature, bolometric flux,
and Gaia distance, we determined the stellar radius using the
Stefan-Boltzmann law.  Combining this radius with the spectroscopic
$\log g$ also yields a stellar mass.
...
poorly understood systematics,  we adopted the stellar parameters
from Method 2, and report them in Table~\ref{tab:starparams}.
...
\begin{figure}[!t]
    ...
    corresponds to a planet size between $0.65\,R_{\rm Jup}$ and
    $0.91\,R_{\rm Jup}$ (3$^{\rm rd}$--97$^{\rm th}$ percentile).
...
Figure~\ref{fig:tessphot}.  We then trimmed the TESS data to windows of
$\pm 7\,{\rm hr}$ centered on each transit.

Our joint model for the TESS and ground-based data was an
...
allowed within each window.  Select parameters and priors
...
There were also three free trend parameters for each of the twelve
transits to account for the rotational variability.  This yielded a
total of 44 free parameters, of which 8 have physical relevance, and
36 are nuisance parameters.  To keep the table at a manageable length,
we only report the physically relevant parameters and derived
quantities.
...
its own did not show evidence for a planetary signal, {\it ii)} we
expected that this approach would provide the best measurement of the
planetary ephemeris, which is key for follow-up efforts, and {\it
iii)} the assumption of identical limb-darkening across all bandpasses
seemed justified by the level of precision in the ground-based data.
The assumption of a constant radius across all bandpasses was tested
...
Table~\ref{tab:posterior}.  The transit is grazing.  In the absence of
a strong prior on the stellar density, the grazing geometry would
hinder our ability to infer the planet-to-star size ratio.  However,
our priors on the stellar parameters from the cluster isochrone fit
are strong enough that we can derive two-sided limits on the
planet-to-star size ratio.  The relevant posterior probabilities are
shown in Figure~\ref{fig:subsetcorner}.
...
% TODO: decide whether to include this. it's not necessary!
...
challenging, for a 1.84$\,$hr transit of a $V=10.6$ star, a detection
...
would greatly constrain a Gaussian process model for the
