...
\subsubsection{Archival Imaging}
...
Ignoring color photometry,
the most plausible HEB scenarios for TOI 837 involve pairs of eclipsing M dwarfs. 

\paragraph{Simple Example}
% SEE June 1, 2020 email in "Re: TOI-837.01" thread
As a simple example,
assume $M_2=M_3=0.2M_\odot$ in a twin HEB.
Assume PMS evolution, and convert between Mstar, delta mag, and Teff using MIST isochrones for a 50 Myr system. 
Take transmission functions (filters only; no instrument or atmosphere response) from the SVO filter profile service (CITE).
Integrate up blackbody functions over each bandpass for each star, and get estimates of the eclipse depths if there were a total eclipse (of either of the secondary or tertiary) with this set of parameters.
The apparent eclipse depths in each bandpass were:
...
The relative depths are what are important (geometry lets them all be scaled to match the observed TESS depth of 40 ppt). The attached plot shows each bandpass, where the narrow ones are as in the order above, and the wide one is TESS.

U and B are ~10x shallower than TESS-band, because the M dwarf blackbody function turns over at ~2x longer wavelengths than the G dwarf blackbody (Wien's law).

Therefore if TOI 837 were a hierarchical eclipsing M dwarf pair, Phil's two R-band observations should have shown eclipses a factor of 1.64x shallower than TESS observed (so, 24ppt, instead of the ~40ppt observed). So, the 40 (+/- 10ish) ppt R-band depths seen in Phil's transits vs TESS-band argue for a planet, but the level of confidence could probably be improved with U or B.

\paragraph{Complicated Implementation}
TODO, probably after U or B-band obtained.
