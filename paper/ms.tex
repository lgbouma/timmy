...
\begin{abstract}
  ...
  years old.  The planet is warm ($P = 8.3\,{\rm d}$) and roughly
  Jupiter-sized.  Combining the photometry with a prior on the stellar
  parameters derived from the cluster color-magnitude diagram, we find
  that the transits are grazing with $b=0.951\pm0.012$ and $R_{\rm p}
  = 0.77 \pm 0.07 \,R_{\rm Jup}$.  From radial velocity monitoring, we
  limit the minimum mass to less than 1.20 $M_{\rm Jup}$ (3$\sigma$).
  Grazing transits are a cause for concern, as they are usually
  indicative of astrophysical false positive scenarios.  Our follow-up
  data show that such scenarios are highly unlikely.  Our combined
  multi-color photometry, high-resolution imaging, and radial
  velocities rule out hierarchical eclipsing binary scenarios.
...
\begin{figure*}[!t]
  ...
  \caption{
    ...
    (Table~\ref{tab:posterior}).  {\it Bottom:} Multicolor photometry
    from the El Sauce 0.36m; black points are binned to 10-minute
    intervals.  Models show $2\sigma$ lower limits on the transit
...
Stefan-Boltzmann law.  Combining this radius the spectroscopic $\log
g$ also yields a stellar mass.
...
\begin{figure}[!t]
    ...
    size ratio, and stellar density.} The 3$^{\rm rd}$ and 97$^{\rm
    th}$ percentiles in the planet-to-star size ratio correspond to
    planet sizes $0.65\,R_{\rm Jup}$ and $0.91\,R_{\rm Jup}$.  This
    plot was made using \texttt{corner} \citep{corner_2016}.
...
our prior on the stellar parameters from the cluster isochrone fit is
strong enough that we can derive two-sided limits on the
